...
<aside 
    ...
    <div class="p-6 flex flex-col h-full overflow-hidden">
        ...
        <nav class="flex-1 space-y-1 overflow-y-auto no-scrollbar">
            ...
            <a href="{{ route('dashboard') }}" class="flex items-center gap-4 px-4 py-3 rounded-xl transition-all {{ request()->routeIs('dashboard') ? 'active-link shadow-lg shadow-blue-100' : 'text-slate-600 hover:bg-slate-50' }}">
                <i class="fas fa-th-large w-5 text-center text-sm"></i>
                <span class="font-bold text-[11px] uppercase tracking-wider">Dashboard</span>
            </a>

            <a href="{{ route('presences') }}" class="flex items-center gap-4 px-4 py-3 rounded-xl transition-all {{ request()->routeIs('presences*') ? 'active-link' : 'text-slate-600 hover:bg-slate-50' }}">
            ...
            <a href="{{ route('tasks.index') }}" class="flex items-center gap-4 px-4 py-3 rounded-xl transition-all {{ request()->routeIs('tasks*') ? 'active-link' : 'text-slate-600 hover:bg-slate-50' }}">
                <i class="fas fa-tasks w-5 text-center text-sm"></i>
                <span class="font-bold text-[11px] uppercase tracking-wider">Tâches</span>
            </a>

            <a href="{{ route('conges') }}" class="flex items-center gap-4 px-4 py-3 rounded-xl transition-all {{ request()->routeIs('conges*') ? 'active-link' : 'text-slate-600 hover:bg-slate-50' }}">
                <i class="fas fa-calendar-alt w-5 text-center text-sm"></i>
                <span class="font-bold text-[11px] uppercase tracking-wider">Congés</span>
            </a>

            <a href="{{ route('paie') }}" class="flex items-center gap-4 px-4 py-3 rounded-xl transition-all {{ request()->routeIs('paie*') ? 'active-link' : 'text-slate-600 hover:bg-slate-50' }}">
            ...
            <a href="{{ route('messages') }}" class="flex items-center gap-4 px-4 py-3 rounded-xl transition-all {{ request()->routeIs('messages*') ? 'active-link' : 'text-slate-600 hover:bg-slate-50' }}">
                <i class="fas fa-comments w-5 text-center text-sm"></i>
                <span class="font-bold text-[11px] uppercase tracking-wider">Messages</span>
            </a>

            {{-- BLOC ADMIN : Apparaît uniquement pour l'administrateur --}}
            @if(auth()->user()->isAdmin() || auth()->user()->role_id == 1)
                <div class="pt-4 mt-4 border-t border-slate-100">
                    ...
                    <a href="{{ route('users.index') }}" class="flex items-center gap-4 px-4 py-3 rounded-xl transition-all {{ request()->routeIs('users*') ? 'active-link shadow-lg shadow-blue-100' : 'text-slate-600 hover:bg-slate-50' }}">
            ...
            {{-- SECTION DOCUMENTS --}}
            <div x-data="{ open: {{ request()->is('documents*') ? 'true' : 'false' }} }" class="pt-2">
                ...
                <div x-show="open" x-cloak x-transition class="mt-1 ml-9 border-l border-slate-200 space-y-1">
                    <a href="{{ route('documents.recus') }}" class="block px-4 py-1.5 text-[10px] font-bold uppercase {{ request()->routeIs('documents.recus') ? 'text-blue-600' : 'text-slate-600 hover:text-slate-900' }}">
                        Reçus
                    </a>
                    <a href="{{ route('documents.envoyes') }}" class="block px-4 py-1.5 text-[10px] font-bold uppercase {{ request()->routeIs('documents.envoyes') ? 'text-blue-600' : 'text-slate-600 hover:text-slate-900' }}">
                        Envoyés
                    </a>
                </div>
            </div>
